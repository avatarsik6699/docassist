...
- **Pytest** — `asyncio_mode = "auto"` — do not decorate async tests with `@pytest.mark.asyncio`; it is implicit.
...
- **Role-based access**: `Depends(require_role(UserRole.admin))` from [core/auth.py](core/auth.py). The closure returns a function that raises 403 if the current user's role does not match.
...
- **Request IDs**: middleware in [main.py](main.py) adds an `X-Request-ID` header to every response and injects it into the log record. Do not reinvent this per-router.
...
- `test_engine` — session-scoped async engine. SQLite in-memory by default; if `DATABASE_URL` is set in the env, uses that DB instead.
...
Naming: `tests/test_<resource>.py`. Prefer fixture-style factories over module-level test data.

---

## Mandatory rules (duplicated locally — these are load-bearing)

These two rules are repeated here because an AI editing inside [app/](.) often will not read the root [AGENTS.md](../AGENTS.md) first, and missing either one costs time.
