# Rules of operation of the AI agent during Docassist development
...
7. **E2E First**: For every user-facing feature touched in a phase, add or update a Playwright spec under `frontend/tests/e2e/`. Chromium is the canonical pass/fail browser for the local phase gate in the reference stack.
...
6. For the reference stack local gate, run:
   ```bash
   pnpm test:e2e:lint
   pnpm test:e2e --project=chromium
   ```
   Cross-browser runs and CI execution are optional unless the project explicitly tightens policy.
7. If the project later enables CI E2E, use [docs/E2E_PIPELINE_CHECKLIST.md](../docs/E2E_PIPELINE_CHECKLIST.md) as the rollout guide. It is not a default branch-protection requirement.
8. Manual browser investigation via Playwright CLI is allowed only when explicitly requested by the user/architect. Keep it out of default gate automation and use it to reproduce or inspect issues that deterministic tests did not capture.

## Response Compression Tools (Opt-in)

1. Optional response compression tools (for example Caveman) are allowed only when explicitly requested by the user/architect.
2. Keep compression mode out of default project policy and out of gate automation.
3. Prefer normal response style for phase contracts, architecture decisions, and handoff notes unless the user requests compressed style for that artifact.
4. If compressed/manual investigation reveals actionable issues, convert them into deterministic tests or concrete checklist items in phase docs.
...
The five SDD workflows are defined in `workflow/docs/playbooks/`:

- [`spec-init`](../workflow/docs/playbooks/spec-init.md) — draft or refresh `docs/SPEC.md` from a project brief
- [`phase-init`](../workflow/docs/playbooks/phase-init.md) — scaffold a new `docs/PHASE_XX.md`
- [`phase-gate`](../workflow/docs/playbooks/phase-gate.md) — validate a phase before commit (stack commands in [docs/STACK.md](../docs/STACK.md#gate-commands))
- [`spec-sync`](../workflow/docs/playbooks/spec-sync.md) — propagate a `docs/SPEC.md` change
- [`context-update`](../workflow/docs/playbooks/context-update.md) — finalize a completed phase
...
- **Other runtimes**: follow the markdown procedure in `workflow/docs/playbooks/` manually.

The runtime wrappers are thin stubs — all workflow logic lives in `workflow/docs/playbooks/`.
