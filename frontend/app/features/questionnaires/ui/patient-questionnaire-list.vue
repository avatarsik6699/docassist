...
<template>
  <section class="space-y-4">
    <div class="space-y-2">
      <p class="eyebrow">Questionnaires</p>
      <h2 class="text-2xl font-semibold tracking-tight text-slate-950">Pending tasks</h2>
      <p class="max-w-2xl text-sm leading-6 text-slate-600">
        Complete assigned questionnaires to keep your doctor updated between visits.
      </p>
    </div>
    ...
    >
      <div v-if="props.isLoading" class="p-6 text-sm text-slate-500">Loading questionnaires…</div>
      <div
        v-else-if="props.items.length === 0"
        class="p-6 text-sm text-slate-500"
        data-testid="patient-questionnaire-empty"
      >
        No pending questionnaires right now.
      </div>
